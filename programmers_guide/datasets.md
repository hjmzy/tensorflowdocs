...
`Dataset.batch()`. See the documentation for [`tf.data.Dataset`](../api_docs/python/tf/data/Dataset.md)
...
A **feedable** iterator can be used together with [`tf.placeholder`](../api_docs/python/tf/placeholder.md) to select
what `Iterator` to use in each call to [`tf.Session.run`](../api_docs/python/tf/Session.md#run), via the familiar
...
[`tf.data.Iterator.from_string_handle`](../api_docs/python/tf/data/Iterator.md#from_string_handle) to define a feedable iterator
...
in [Regression Examples](../get_started/linear_regression.md).
...
The [`tf.train.MonitoredTrainingSession`](../api_docs/python/tf/train/MonitoredTrainingSession.md) API simplifies many aspects of running
TensorFlow in a distributed setting. `MonitoredTrainingSession` uses the
[`tf.errors.OutOfRangeError`](../api_docs/python/tf/errors/OutOfRangeError.md) to signal that training has completed, so to use it
...
To use a `Dataset` in the `input_fn` of a [`tf.estimator.Estimator`](../api_docs/python/tf/estimator/Estimator.md), we also
