...
input pipeline issues and best practices. We found that using [`tf.FIFOQueue`](../api_docs/python/tf/FIFOQueue.md)
and [`tf.train.queue_runner`](../api_docs/python/tf/train/queue_runner.md) could not saturate multiple current generation GPUs
...
similar to [`tf.FIFOQueue`](../api_docs/python/tf/FIFOQueue.md). The difference is that `StagingArea`  does not 
...
tensors each with a batch-size of 32. Rather than using [`tf.concat`](../api_docs/python/tf/concat.md) for this
purpose, which is implemented as a single op that waits for all the inputs to be
ready before concatenating them together, [`tf.parallel_stack`](../api_docs/python/tf/parallel_stack.md) is used.
[`tf.parallel_stack`](../api_docs/python/tf/parallel_stack.md) allocates an uninitialized tensor as an output, and each
...
`StagingArea` is a queue-like operator similar to [`tf.FIFOQueue`](../api_docs/python/tf/FIFOQueue.md) that offers
...
performance on GPU. Below is an example of using [`tf.contrib.layers.batch_norm`](../api_docs/python/tf/contrib/layers/batch_norm.md)
...
However, we can instead use the optional NCCL ([`tf.contrib.nccl`](../api_docs/python/tf/contrib/nccl.md)) support. NCCL
