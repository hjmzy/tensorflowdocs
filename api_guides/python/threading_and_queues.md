...
out" queue ([`tf.FIFOQueue`](../../api_docs/python/tf/FIFOQueue.md)) and fill it with zeros.  Then we'll construct a
...
Queues, such as [`tf.FIFOQueue`](../../api_docs/python/tf/FIFOQueue.md)
and [`tf.RandomShuffleQueue`](../../api_docs/python/tf/RandomShuffleQueue.md),
...
We recommend using the [`shuffle`](../../api_docs/python/tf/data/Dataset.md#shuffle)
and [`batch`](../../api_docs/python/tf/data/Dataset.md#batch) methods of a
[`Dataset`](../../api_docs/python/tf/data/Dataset.md) to accomplish this. However, if you'd prefer
to use a queue-based version instead, you can find a full implementation in the
[`tf.train.shuffle_batch`](../../api_docs/python/tf/train/shuffle_batch.md) function.
...
Once started by [`tf.train.start_queue_runners`](../../api_docs/python/tf/train/start_queue_runners.md), or indirectly through
[`tf.train.MonitoredSession`](../../api_docs/python/tf/train/MonitoredSession.md), the `QueueRunner` will launch the
...
by [`tf.train.MonitoredSession`](../../api_docs/python/tf/train/MonitoredSession.md) is sufficient. In the rare case that it is not,
...
[`tf.train.Coordinator`](../../api_docs/python/tf/train/Coordinator.md) and
[`tf.train.QueueRunner`](../../api_docs/python/tf/train/QueueRunner.md). These two classes
...
The [`tf.train.Coordinator`](../../api_docs/python/tf/train/Coordinator.md) class manages background threads in a TensorFlow
program and helps multiple threads stop together.

Its key methods are:

* [`tf.train.Coordinator.should_stop`](../../api_docs/python/tf/train/Coordinator.md#should_stop): returns `True` if the threads should stop.
* [`tf.train.Coordinator.request_stop`](../../api_docs/python/tf/train/Coordinator.md#request_stop): requests that threads should stop.
* [`tf.train.Coordinator.join`](../../api_docs/python/tf/train/Coordinator.md#join): waits until the specified threads have stopped.
...
also has support to capture and report exceptions.  See the [`tf.train.Coordinator`](../../api_docs/python/tf/train/Coordinator.md) documentation for more details.

### QueueRunner

The [`tf.train.QueueRunner`](../../api_docs/python/tf/train/QueueRunner.md) class creates a number of threads that repeatedly
