...
5.  Test the op. We usually do this in Python for convenience, but you can also
    ...
    Python [gradient checker](../api_docs/python/tf/test/compute_gradient_error.md).
    See
...
[`tf.load_op_library`](../api_docs/python/tf/load_op_library.md) function to
...
[tensor types](../api_docs/python/tf/DType.md) used for inputs and outputs.)
...
* `{<type1>, <type2>}`: The value is of type `type`, and must be one of
  ...
  [tensor types](../api_docs/python/tf/DType.md).  You don't specify
...
use [the `DT_*` names for the types](../api_docs/python/tf/DType.md).
...
* `<type>`, where `<type>` is a supported input type (e.g. `float`, `int32`,
  ...
  See
  [the list of supported Tensor types](../api_docs/python/tf/DType.md).
...
* For a sequence of tensors with the same type: `<number> * <type>`, where
  ...
  either be
  [a specific type like `int32` or `float`](../api_docs/python/tf/DType.md),
...
[described here](../programmers_guide/version_compat.md#compatibility_of_graphs_and_checkpoints).
...
[`GraphDef` version semantics](../programmers_guide/version_compat.md#compatibility_of_graphs_and_checkpoints).
...
For example, the [`tf.pad`](../api_docs/python/tf/pad.md) has
...
[`tf.RegisterGradient`](../api_docs/python/tf/RegisterGradient.md):

* For an op with one output, the gradient function will take an
  [`tf.Operation`](../api_docs/python/tf/Operation.md) `op` and a
  [`tf.Tensor`](../api_docs/python/tf/Tensor.md) `grad` and build new ops
  out of the tensors
  ...
  [`tf.Operation.get_attr`](../api_docs/python/tf/Operation.md#get_attr).
