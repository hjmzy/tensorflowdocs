...
*   [`tf.estimator.LinearClassifier`](../api_docs/python/tf/estimator/LinearClassifier.md):
    Constructs a linear classification model.
*   [`tf.estimator.LinearRegressor`](../api_docs/python/tf/estimator/LinearRegressor.md):
    Constructs a linear regression model.
*   [`tf.estimator.DNNClassifier`](../api_docs/python/tf/estimator/DNNClassifier.md):
    Construct a neural network classification model.
*   [`tf.estimator.DNNRegressor`](../api_docs/python/tf/estimator/DNNRegressor.md):
    Construct a neural network regression model.
*   [`tf.estimator.DNNLinearCombinedClassifier`](../api_docs/python/tf/estimator/DNNLinearCombinedClassifier.md):
    Construct a neural network and linear combined classification model.
*   [`tf.estimator.DNNLinearCombinedRegressor`](../api_docs/python/tf/estimator/DNNLinearCombinedRegressor.md):
...
*   [tf.estimator Quickstart](../get_started/estimator.md): Quick introduction to
    training a neural network using tf.estimator.
*   [TensorFlow Linear Model Tutorial](../tutorials/wide.md): Introduction to
    feature columns, and an overview on building a linear classifier in
    tf.estimator.
*   [Building Input Functions with tf.estimator](../get_started/input_fn.md): Overview of how
...
*   `labels`: A `Tensor` containing the labels passed to the model via
    ...
    model will infer.
*   `mode`: One of the following [`tf.estimator.ModeKeys`](../api_docs/python/tf/estimator/ModeKeys.md) string values
...
The `model_fn` must return a [`tf.estimator.EstimatorSpec`](../api_docs/python/tf/estimator/EstimatorSpec.md)
...
*   `eval_metric_ops` (optional). A dict of name/value pairs specifying the
    ...
    your metric calculation. The [`tf.metrics`](../api_docs/python/tf/metrics.md)
...
If `features` contains a dict of [feature columns](../tutorials/linear.md#feature-columns-and-transformations) passed to
the model via an input function, you can convert it to an input-layer `Tensor`
with the [`tf.feature_column.input_layer`](../api_docs/python/tf/feature_column/input_layer.md) function.
...
*   `tf.nn.relu6`. The following code creates a layer of `units` nodes fully
    ...
    function ([`tf.nn.relu6`](../api_docs/python/tf/nn/relu6.md)):
...
([`tf.sigmoid`](../api_docs/python/tf/sigmoid.md)). For a list of predefined
activation functions available in TensorFlow, see the [API docs](../api_guides/python/nn.md#activation_functions).
...
[`tf.reshape`](../api_docs/python/tf/reshape.md) to a one-dimensional
...
reflect the label values during training and evaluation runs. The [`tf.losses`](../api_docs/python/tf/losses.md)
...
See the [API guide](../api_guides/python/contrib.losses.md) for a
...
[`tf.train.get_global_step`](../api_docs/python/tf/train/get_global_step.md) takes care of generating an integer variable:
...
[API guide](../api_guides/python/train.md#optimizers).
...
[Building Input Functions with tf.estimator](../get_started/input_fn.md).
...
*   [Layers](../api_guides/python/contrib.layers.md)
*   [Losses](../api_guides/python/contrib.losses.md)
*   [Optimization](../api_guides/python/contrib.layers.md#optimization)
